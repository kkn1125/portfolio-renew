...
export const Information = {
  ...
  coreCompetencies: [
    "Node WebSocket 기반 실시간 멀티플레이 서버 구축",
    "React·API·게임 API·백오피스 이중화 운영 및 배포",
    "프로젝트 운영, 일정 조율, 추가 요청 대응",
    "DB 버전 업그레이드 및 개발·검증·운영 환경 이관",
    "온프레미스 인프라 구축(NAS, MariaDB·PostgreSQL·Redis, SVN 이전)",
    "CI/CD 이관 협의 및 자동화",
    "다협력사 협업 및 문서화·소통",
  ],
  description: [
    "서비스의 확장성과 비즈니스 로직에 최적화된 백엔드 설계 및 운영 경험을 쌓아왔습니다. Cardtalk 프로젝트에서 Node WebSocket 서버를 직접 설계·구현했고, 구축 이후 React 프론트엔드·백엔드 API·게임 전용 API·백오피스가 모두 이중화된 운영 환경에서 일정 조율과 추가 요청 대응을 포함한 운영 전반을 담당하고 있습니다. API 성능 최적화, DB 버전 업그레이드를 위한 개발·검증·운영 서버 이관, CI/CD 이관 협의와 일정 조율을 통해 배포 및 운영 효율성을 개선해온 경험이 있습니다.",
    "사내 NAS 서버를 담당하며 클라우드 서버와 블록 스토리지에 있던 리소스를 온프레미스로 이관했고, MariaDB·PostgreSQL·Redis 등 사내 데이터베이스를 구축하고 외부 SVN 서버까지 사내로 이전하는 작업을 수행했습니다. React, NestJS, Spring Boot는 물론 Golang·Python까지 폭넓게 활용하며, 개인 서버 기반 릴레이·업무 보조·REST API·시나리오 테스트 도구, 에이전트와 MCP를 결합한 프로젝트별 이슈·업무 프로세스 자동화로 업무 능률을 높이고 있습니다.",
    "협업에 필요한 문서화와 소통을 중요하게 생각하며, 기획에 정확하게 부합하는 기능 개발을 위해 이해와 소통을 우선합니다. 부족함을 느끼며 필요한 지식을 학습하고, 문제 해결 과정을 기록하며 성장하고 있습니다.",
  ],
  resume: [
    new Resume("프로젝트 운영 및 인프라 구축 경험", [
      "Cardtalk 프로젝트에서 Node WebSocket 서버를 담당하여 실시간 멀티플레이 카드 게임 백엔드를 설계·개발했습니다. Redis 분산락과 Lua 스크립트 기반 게임 로직, 타이머 기반 저장·만료 제어, Loki·Grafana 연동 로그 중앙화, 부하 테스트 계획 수립 및 결과 분석까지 게임 서버 구축과 관측성 체계 마련까지 수행했습니다.",

      "구축 이후 Cardtalk 운영을 담당하며 React 프론트엔드, 백엔드 API, 게임 전용 API, 백오피스 서버가 모두 이중화된 환경에서 배포·장애 대응·모니터링 등 운영 건 전반을 직접 처리하고 있습니다. 팀과 이해관계자와 직접 소통하며 일정 조율과 추가 요청에 대한 대응을 주도하고 있습니다.",

      "DB 버전 업그레이드를 위해 개발·검증·운영 서버의 데이터베이스 이관을 기획하고 실행하여 무사히 마쳤습니다. 스키마 변경과 데이터 호환성을 검증하는 과정을 거쳤으며, CI/CD 이관을 위한 협의와 일정 조율도 함께 진행했습니다.",

      "사내 NAS 서버를 담당하며 기존 클라우드 서버와 블록 스토리지에 분산되어 있던 리소스를 사내 환경으로 이관하는 작업을 수행했습니다. MariaDB, PostgreSQL, Redis 등 사내에서 사용하는 데이터베이스를 직접 구축·운영했고, 외부 서버에서 사용 중이던 SVN 서버까지 사내 서버로 이전하여 개발·운영 인프라를 일원화했습니다.",

      "DalDaleng 프로젝트에서는 Spring Boot API, 백오피스, 배치, 캡처 서버를 담당하며 4개 협력사와 소통하면서 개발·운영을 진행하고 있습니다. 다자간 협업 환경에서 요구사항 정리, 일정 조율, 운영 이슈 대응을 병행하며 서비스 안정성을 유지하고 있습니다.",

      "업무 능률 향상을 위해 개인 서버를 구축하여 원격 제어용 릴레이 서버, 업무 보조 서비스, REST API·시나리오 테스트 도구를 운영하고 있습니다. 에이전트와 MCP를 결합해 프로젝트별 주요 이슈와 업무 프로세스를 지원하는 비서 서비스 및 자동화 체계를 직접 만들어 실무에 적용하고 있습니다.",
    ]),
